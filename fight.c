...
int compute_armor_class(struct char_data *ch, struct char_data *att)
{
  ...
  armorclass += get_size_bonus(get_size(ch));

  if (IS_AFFECTED(ch, AFF_STUNNED))
    armorclass -= 2;
  else if (GET_POS(ch) == POS_FIGHTING) {
    armorclass += ability_mod_value(GET_DEX(ch));
...
void make_corpse(struct char_data *ch)
{
  ...
  snprintf(buf2, sizeof(buf2), "The corpse of %s@[2] is lying here.", GET_NAME(ch));
...
void change_alignment(struct char_data *ch, struct char_data *victim)
{
  ...
  GET_ALIGNMENT(ch) += (-GET_ALIGNMENT(victim) - GET_ALIGNMENT(ch)) / cmdiv;
}
...
void solo_gain(struct char_data *ch, struct char_data *victim)
{
  ...
  if (GET_LEVEL(ch) <= GET_LEVEL(victim)) {
    ...
    if (IS_NPC(ch)) {
      exp -= MAX(0, (exp * MIN(4, (GET_LEVEL(ch) - GET_LEVEL(victim)))) / 8);
    } else {
      exp -= MAX(0, (exp * MIN(8, (GET_LEVEL(ch) - GET_LEVEL(victim)))) / 8);
    }
  }
...
void damage_object(struct char_data *ch, struct char_data *victim)
{
  ...
  snum = GET_DEX(ch) + rand_number(1, 20);

  rnum = GET_DEX(victim) + rand_number(1, 20);
...
int one_hit(struct char_data *ch, struct char_data *victim, struct obj_data *wielded, int w_type, int calc_base_hit, char *damstr, char *critstr, int hitbonus)
{
  ...
  if (!dam) {
    ...
    /* Remove stunning fist if affected */
    if (IS_AFFECTED(ch, AFF_STRIKE_STUNNING)) {
      REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_STRIKE_STUNNING);
    }
    ...
    /* If stunning fist strike, do it. Victim save etc was already checked. */   
    if (IS_AFFECTED(ch, AFF_STRIKE_STUNNING)) {
      af.type = FEAT_STUNNING_FIST;
      af.duration = 10;
      af.modifier = 0;
      af.location = APPLY_NONE;
      af.bitvector = AFF_STUNNED;
      af.sktype = SKTYPE_FEAT;
      affectv_join(victim, &af, FALSE, FALSE, FALSE, FALSE);
      if (GET_EQ(victim, WEAR_WIELD1)) {
        obj = (GET_EQ(victim, WEAR_WIELD1));
        perform_remove(victim, WEAR_WIELD1);
        perform_drop(victim, obj, SCMD_DROP, "drop", 0);
      }
      if (GET_EQ(victim, WEAR_WIELD2)) {
        obj = (GET_EQ(victim, WEAR_WIELD2));
        perform_remove(victim, WEAR_WIELD2);
        perform_drop(victim, obj, SCMD_DROP, "drop", 0);
      }
    }
    /* Remove stunning fist if affected */
...
void hit(struct char_data *ch, struct char_data *victim, int type)
{
  ...
  if (IS_AFFECTED(ch, AFF_STUNNED)) {
    mudlog(BRF, ADMLVL_IMMORT, TRUE, "DEBUG: %s is stunned.", GET_NAME(ch));
    return;
  }
