...
int can_take_obj(struct char_data *ch, struct obj_data *obj)
{
  if (AFF_FLAGGED(ch, AFF_SPIRIT)) {
    ...
    return(0);
  } else if (!(CAN_WEAR(obj, ITEM_WEAR_TAKE)) && (GET_ADMLEVEL(ch) == ADMLVL_NONE)) {
    act("$p: you can't take that!", FALSE, ch, obj, 0, TO_CHAR);
    return(0);
  } else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch) && !OBJ_FLAGGED(obj, ITEM_BROKEN)) {
...
ACMD(do_drink)
{
  ...
  if (!GET_OBJ_VAL(temp, VAL_DRINKCON_HOWFULL) && (!GET_OBJ_VAL(temp, VAL_DRINKCON_CAPACITY) == 1)) {
  ...
  gain_condition(ch, HUNGER, drink_aff[GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID)][HUNGER] * amount / 4);
...
ACMD(do_pour)
{
  ...
  if (GET_OBJ_VAL(from_obj, VAL_DRINKCON_CAPACITY) > 0) {
    GET_OBJ_VAL(from_obj, VAL_DRINKCON_HOWFULL) -= 
      (amount = (GET_OBJ_VAL(to_obj, VAL_DRINKCON_CAPACITY) - GET_OBJ_VAL(to_obj, VAL_DRINKCON_HOWFULL)));
    GET_OBJ_VAL(to_obj, VAL_DRINKCON_HOWFULL) = GET_OBJ_VAL(to_obj, VAL_DRINKCON_CAPACITY);
...
int find_eq_pos(struct char_data *ch, struct obj_data *obj, char *arg)
{
  ...
  if (!arg || !*arg) {
    ...
    if (CAN_WEAR(obj, ITEM_WEAR_WINGS) && BODY_FLAGGED(ch, WEAR_WINGS))      return WEAR_WINGS;
    if (CAN_WEAR(obj, ITEM_WEAR_MASK) && BODY_FLAGGED(ch, WEAR_MASK))        return WEAR_MASK;
