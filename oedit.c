...
void oedit_disp_prompt_spellbook_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < MAX_SPELL_LEVEL; i++) {
    ...
    write_to_output(d, "@c---@wLevel @R%d @wSpells@c---===============================================------@n\r\n", i);
    for (counter = 0; counter < SPELLBOOK_SIZE; counter++) {
      if (OLC_OBJ(d)->sbinfo && OLC_OBJ(d)->sbinfo[counter].spellname != 0 && OLC_OBJ(d)->sbinfo[counter].spellname < MAX_SPELLS 
          && spell_info[OLC_OBJ(d)->sbinfo[counter].spellname].spell_level == i) {
        write_to_output(d, " @g%3d@n) %-20.20s %s", counter + 1, spell_info[OLC_OBJ(d)->sbinfo[counter].spellname].name, !(++columns % 3) ? "\r\n" : "");
        u++;
      }
    }
  }
...
void oedit_disp_spellbook_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < MAX_SPELL_LEVEL; i++) {
    ...
    for (counter = 0; counter < SKILL_TABLE_SIZE; counter++) {
      if (spell_info[counter].skilltype == SKTYPE_SPELL && spell_info[counter].spell_level == i &&
          spell_info[counter].school != SCHOOL_UNDEFINED) 
        write_to_output(d, "@g%3d@n) @y%-20.20s@n%s", counter, spell_info[counter].name, !(++columns % 3) ? "\r\n" : "");
    }
  }
...
void oedit_disp_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d,
	  ...
	  "@g8@n) Weight      : @c%-4d@n, 	@g9@n) Cost        : @c%-4d@n\r\n"
