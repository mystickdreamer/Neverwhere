...
void list_mobiles(struct char_data *ch, zone_rnum rnum, zone_vnum vmin, zone_vnum vmax)
{
  ...
  send_to_char(ch,
  "Index VNum    Mobile Name                                   Level Avg. Dmg\r\n"
  "----- ------- --------------------------------------------- --------------\r\n");
  ...
  for (i = 0; i <= top_of_mobt; i++) {
    if (mob_index[i].vnum >= bottom && mob_index[i].vnum <= top) {
      ...
      send_to_char(ch, "@g%4d@n) [@g%-5d@n] @[3]%-*s @y[%4d]@n @y[%4d]@n %s\r\n",      
                   counter, mob_index[i].vnum, count_color_chars(mob_proto[i].short_descr)+44, mob_proto[i].short_descr,
                   mob_proto[i].level + mob_proto[i].level_adj + mob_proto[i].race_level, admg,
