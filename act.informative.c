...
int show_obj_modifiers(struct obj_data *obj, struct char_data *ch) {
	...
	if (OBJ_FLAGGED(obj, ITEM_BLESS) && AFF_FLAGGED(ch, AFF_DETECT_ALIGN)) {
		send_to_char(ch, " ..@BIt glows blue@n!");
		found++;
	}
	if (OBJ_FLAGGED(obj, ITEM_MAGIC) && AFF_FLAGGED(ch, AFF_DETECT_MAGIC)) {
		send_to_char(ch, " ..@YIt glows yellow@n!");
		found++;
	}
	if (OBJ_FLAGGED(obj, ITEM_GLOW)) {
		send_to_char(ch, " ..@yIt has a soft glowing aura@n!");
		found++;
	}
...
void look_at_char(struct char_data *i, struct char_data *ch) {
	...
	send_to_char(ch, "%c%s is %s sized.\r\n", UPPER(*GET_NAME(i)), GET_NAME(i) + 1, size_names[get_size(i)]);

	diag_char_to_char(i, ch);

	if (RIDING(i) && IN_ROOM(RIDING(i)) == IN_ROOM(i)) {
		if (RIDING(i) == ch)
			send_to_char(ch, "%s is mounted on you.\r\n", GET_NAME(ch));
		else {
			send_to_char(ch, "%s is mounted upon %s.\r\n", GET_NAME(i), PERS(RIDING(i), ch));
		}
	} else if (RIDDEN_BY(i) && IN_ROOM(RIDDEN_BY(i)) == IN_ROOM(i)) {
		if (RIDDEN_BY(i) == ch)
			send_to_char(ch, "You are mounted upon %s.\r\n", HMHR(i));
		else {
			send_to_char(ch, "%c%s is mounted by %s\r\n", UPPER(*GET_NAME(i)), GET_NAME(i) + 1, PERS(RIDDEN_BY(i), ch));
		}
	}
...
void list_one_char(struct char_data *i, struct char_data *ch) {
	...
	if (IS_NPC(i) && i->long_descr && GET_POS(i) == GET_DEFAULT_POS(i)) {
		...
		if (RIDDEN_BY(i) == ch && (IN_ROOM(i) == IN_ROOM(ch))) {
			send_to_char(ch, "...mounted by you.\r\n");
		}
...
void look_at_room(room_rnum target_room, struct char_data *ch, int ignore_brief) {
	...
	if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_ROOMFLAGS)) {
		...
		send_to_char(ch, "@[1][%5d] ", GET_ROOM_VNUM(target_room));
		...
		send_to_char(ch, "%s [ %s] [ %s ]@[0]\r\n", world[target_room].name, buf, buf2);
	} else
	...
	if ((!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_BRIEF)) || ignore_brief ||
		...
		if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_AUTOMAP) && can_see_map(ch)) {
			str_and_map(world[target_room].description, ch);
		} else {
...
ACMD(do_skills) {
	...
	if (*name = "weapon") {
		...
		send_to_char(ch, "@r=-  @cSmall Edged@n:@Y%5d  @cSmall Blunt@n:@Y%5d @cShortBow@n:@Y%5d @cLight Crossbow@n:@Y%5d @cLight Thrown@n:@Y%5d\r\n",
		...
		send_to_char(ch, "@r=- @cMedium Edged@n:@Y%5d @cMedium Blunt@n:@Y%5d  @cLongBow@n:@Y%5d @cHeavy Crossbow@n:@Y%5d @cHeavy Thrown@n:@Y%5d\r\n", 
		...
		send_to_char(ch, "@r=-  @cHeavy Edged@n:@Y%5d  @cHeavy Blunt@n:@Y%5d\r\n @cPolearm@n:@Y%5d @cBackstab@n:@Y%5d\r\n", 
...
ACMD(do_score) {
	...
	send_to_char(ch, "@r=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=@BGold@r-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=@n\r\n");
	send_to_char(ch, "       @cOn Hand@n:@Y%d               @cBank@n: @Y%d@n\r\n", GET_GOLD(ch), GET_BANK_GOLD(ch));
	...
	if (RIDING(ch)) {
		send_to_char(ch, "You are mounted on %s.\r\n", PERS(RIDING(ch), ch));
	}
	...
	send_to_char(ch, "@CSkills@n:@cto see your skills@n\r\n");
		...
		if (isdigit(*arg)) {
			...
			switch (mode) {
				...
				case 'q':
					...
					strcpy(buf, buf1); /* strcpy: OK (sizeof: buf1 == buf) */
					break;
				case 'r': /* shouldn't you just use look? */
					who_room = 1;
					strcpy(buf, buf1); /* strcpy: OK (sizeof: buf1 == buf) */
					break;
				case 's':
...
ACMD(do_levels) {
	...
	for (i = 1; i < CONFIG_LEVEL_CAP; i++) {
		if (i == CONFIG_LEVEL_CAP - 1)
			...
			nlen = snprintf(buf + len, sizeof (buf) - len, "[%2"SZT"] %8d-%-8d : \r\n", i,
