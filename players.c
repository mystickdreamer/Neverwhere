...
extern struct pclean_criteria_data pclean_criteria[7];
...
void build_player_index(void)
{
  ...
  for (i = 0; i < rec_count; i++) {
    ...
    sscanf(line, "%ld %s %d %s %ld %d", &player_table[i].id, arg2,
      &player_table[i].level, bits, &player_table[i].last, &player_table[i].admlevel);
...
void load_damreduce(struct char_data *ch, FILE *fl)
{
  ...
  if (sscanf(buf, "%d %d %d", &spell, &feat, &mod) != 3) {
...
int load_char(const char *name, struct char_data *ch)
{
  ...
  if ((id = get_ptable_by_name(name)) < 0)
    ...
    if (!get_filename(cmfname, sizeof(cmfname), PLR_FILE, player_table[id].name))
      return (-1);
    if (!(fl = fopen(cmfname, "r"))) {
      mudlog(NRM, ADMLVL_GOD, TRUE, "SYSERR: Couldn't open player file %s", cmfname);
      return (-1);
    }
    ...
    for (i = 0; i < AD_ARRAY_MAX; i++)
      ADM_FLAGS(ch)[i] = 0;
    for (i = 0; i < AF_ARRAY_MAX; i++)
      BODY_PARTS(ch)[i] = PFDEF_AFFFLAGS;
    ...
    GET_COMBATEXP(ch) = PFDEF_COMBATEXP;
    ...
    while (get_line(fl, line)) {
      ...
      case 'B':
        ...
        /*  Not used yet. We are forcing use of racial lookup table.
         *  But the idea is that each player can maintain their own
         *  body parts as needed.
         *  else if (!strcmp(tag, "Body")) {
         *    sscanf(line, "%s %s %s %s", f1, f2, f3, f4);
         *    BODY_PARTS(ch)[0] = asciiflag_conv(f1);
         *    BODY_PARTS(ch)[1] = asciiflag_conv(f2);
         *    BODY_PARTS(ch)[2] = asciiflag_conv(f3);
         *    BODY_PARTS(ch)[3] = asciiflag_conv(f4);
         *  } */
      ...
      case 'C':
        ...
        else if (!strcmp(tag, "Comb"))  GET_COMBATEXP(ch)       = atoi(line);
      break;
      ...
      case 'M':
        ...
        else if (!strcmp(tag, "Murd"))  GET_MURDER(ch)          = atoi(line);
      ...
      case 'T':
             ...
             if (!strcmp(tag, "Thr1"))  GET_SAVE_MOD(ch, 0)     = atoi(line);
  ...
  /* Can be turned on to log when players unrent but don't have a body part 
   * profile saved in their player file. Forces lookup of racial template. 
   * After player has profile saved they won't hit this any more. */
  if (! BODY_PARTS(ch)[0]) {
    /* log("No body parts by race for %s - %s, applying race template.", pc_race_types[(int)GET_RACE(ch)], GET_NAME(ch)); */
  ...
  if (GET_ADMLEVEL(ch) >= ADMLVL_IMMORT) {
    for (i = 1; i <= SKILL_TABLE_SIZE; i++) {
      SET_SKILL(ch, i, 100);
      SET_SKILL_CLASS(ch, i, SKLEARN_CLASS);
    }
...
void save_char_pets(struct char_data *ch)
{
  ...
  if (!get_filename(cmfname, sizeof(cmfname), PET_FILE, GET_NAME(ch)))
    return;

  if (!(fl = fopen(cmfname, "w"))) {
    mudlog(NRM, ADMLVL_GOD, TRUE, "SYSERR: Couldn't open pet file %s for write", cmfname);
    return;
  }
...
{
  FILE *fl;
  char cmfname[40], buf[MAX_STRING_LENGTH];
  ...
  if (!get_filename(cmfname, sizeof(cmfname), PLR_FILE, GET_NAME(ch)))
    return;
  if (!(fl = fopen(cmfname, "w"))) {
    mudlog(NRM, ADMLVL_GOD, TRUE, "SYSERR: Couldn't open player file %s for write", cmfname);
    return;
  }
  ...
  if (GET_MURDER(ch)	   != PFDEF_MURDER)	fprintf(fl, "Murd: %d\n", GET_MURDER(ch));
  ...
  /* Not used yet. This will force a save of player body parts to
   * their player file. Can then be used to load body parts info
   * for a unique character instead of using racial template lookups.
   * sprintascii(fbuf1, BODY_PARTS(ch)[0]);
   * sprintascii(fbuf2, BODY_PARTS(ch)[1]);
   * sprintascii(fbuf3, BODY_PARTS(ch)[2]);
   * sprintascii(fbuf4, BODY_PARTS(ch)[3]);
   * fprintf(fl, "Body: %s %s %s %s\n", fbuf1, fbuf2, fbuf3, fbuf4);*/
  ...
  if (GET_COMBATEXP(ch)    != PFDEF_COMBATEXP)  fprintf(fl, "Comb: %d\n", GET_COMBATEXP(ch));
...
void remove_player_cwg(int pfilepos)
{
  char cmfname[40];
  int i;
  ...
  /* Update top_of_p_table so things like players command is correct. */
  top_of_p_table -= 1;

  /* Unlink all player-owned files */
  for (i = 0; i < MAX_FILES; i++) {
    if (get_filename(cmfname, sizeof(cmfname), i, player_table[pfilepos].name))
      unlink(cmfname);
  }
