...
void current_update(void) {
	...
	for (i = character_list; i; i = next_char) {
		...
		if ((GET_POS(i) == POS_STANDING) && ROOM_FLAGGED(IN_ROOM(i), ROOM_CURRENT) &&
			...
			send_to_char(i, "The strong current carries you %s!\r\n", dirs[door]);
			act("$n is taken $T by the rough current!", FALSE, i, 0, dirs[door], TO_NOTVICT);
...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check) {
	...
	if ((SECT(IN_ROOM(ch)) == SECT_UNDERWATER) ||
		...
		if (!has_o2(ch)) {
			send_to_char(ch, "You wouldn't be able to breathe there!\r\n");
			return (0);
		}
	}
	...
	if (GET_MOVE(ch) < need_movement && !IS_NPC(ch)) {
		...
		} else if (RIDING(ch) && GET_MOVE(RIDING(ch)) < need_movement) {
	...
	if (!ADM_FLAGGED(ch, ADM_WALKANYWHERE) && !IS_NPC(ch) && !RIDING(ch)) {
		GET_MOVE(ch) -= need_movement;
	} else if (RIDING(ch)) {
	...
	if (RIDING(ch)) {
		for (och = world[IN_ROOM(ch)].people; och; och = och->next_in_room) {
			if (ch != och) {
				if (!IS_NPC(och)) {
					if (IS_NPC(ch)) { /* This NPC is fixed also. */
						snprintf(buf2, sizeof (buf2), "$n rides %s on $N.", dirs[dir]);
						act(buf2, TRUE, ch, 0, 0, TO_ROOM);
						break;
					} else if (GET_RACE(ch) == GET_RACE(och)) {
						snprintf(buf2, sizeof (buf2), "$n rides %s on $N.", dirs[dir]);
	...
	} else { /* This leave while walking code is fixed!! */
		for (och = world[IN_ROOM(ch)].people; och; och = och->next_in_room) {
			if (ch != och) {
				if (!IS_NPC(och)) {
					if ((AFF_FLAGGED(ch, AFF_SNEAK) && PRF_FLAGGED(och, PRF_HOLYLIGHT)) ||
						...
						if (IS_NPC(ch)) {
							...
							snprintf(buf2, sizeof (buf2), "$n leaves %s.", dirs[dir]);
							act(buf2, TRUE, ch, 0, och, TO_VICT);
						} else {
							snprintf(buf2, sizeof (buf2), "$n leaves %s.", dirs[dir]);
							act(buf2, TRUE, ch, 0, och, TO_VICT);
						}
	...
	if (RIDING(ch) && IN_ROOM(RIDING(ch)) != IN_ROOM(ch)) {
		snprintf(buf2, sizeof (buf2), "$n arrives from %s%s, riding $N.",
			((dir == UP || dir == DOWN) ? "" : "the "),
			(dir == UP ? "below" :
			dir == DOWN ? "above" : dirs[rev_dir[dir]]));
		act(buf2, TRUE, ch, 0, RIDING(ch), TO_ROOM | TO_SNEAKRESIST);
		char_from_room(RIDING(ch));
		char_to_room(RIDING(ch), IN_ROOM(ch));
		look_at_room(IN_ROOM(RIDING(ch)), RIDING(ch), 0);
	} else if (RIDDEN_BY(ch) && IN_ROOM(RIDDEN_BY(ch)) != IN_ROOM(ch)) {
		snprintf(buf2, sizeof (buf2), "$n arrives from %s%s, ridden by $N.",
			((dir == UP || dir == DOWN) ? "" : "the "),
			(dir == UP ? "below" :
			dir == DOWN ? "above" : dirs[rev_dir[dir]]));
		act(buf2, TRUE, ch, 0, RIDDEN_BY(ch), TO_ROOM | TO_SNEAKRESIST);
		char_from_room(RIDDEN_BY(ch));
		char_to_room(RIDDEN_BY(ch), IN_ROOM(ch));
		look_at_room(IN_ROOM(RIDDEN_BY(ch)), RIDDEN_BY(ch), 0);
	} else {
...
#define DOOR_KEY(ch, obj, door)		((obj) ? (GET_OBJ_VAL(obj, VAL_KEY_KEYCODE)) : \
					(EXIT(ch, door)->key))
#define DOOR_DCLOCK(ch, obj, door)	((GET_OBJ_LOCK_DC(obj)) ? (GET_OBJ_VAL(obj, VAL_DOOR_DCLOCK)) : EXIT(ch, door)->dclock)
...
ACMD(do_mount) {
	...
	if (arg == NULL || !*arg) {
		...
		send_to_char(ch, "Mounting of other players in not currently allowed.\r\n");
	} else if (RIDING(ch)) {
...
ACMD(do_dismount) {
	...
	} else if (SECT(ch->in_room) == SECT_FLYING && !has_flight(ch)) {
		send_to_char(ch, "Yah, right, and then fall to your death...\r\n");
	} else {
