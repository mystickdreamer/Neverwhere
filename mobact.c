...
void mobile_activity(void)
{
  ...
  for (ch = character_list; ch; ch = next_ch) {
    ...
    /* mounted mob bucking */
    if (RIDDEN_BY(ch) && !AFF_FLAGGED(ch, AFF_TAMED) && GET_ADMLEVEL(RIDDEN_BY(ch)) < ADMLVL_IMMORT) {
      if (GET_CHA(ch) > roll_skill(RIDDEN_BY(ch), SKILL_HANDLE_ANIMAL)) {
        act("$n rears backwards, throwing you to the ground.", FALSE, ch, 0, RIDDEN_BY(ch), TO_VICT);
        act("$n rears backwards, throwing $N to the ground.", FALSE, ch, 0, RIDDEN_BY(ch), TO_NOTVICT);
        damage(RIDDEN_BY(ch), RIDDEN_BY(ch), dice(2, 4), -1, 0, -1, 0, 0, 0);
