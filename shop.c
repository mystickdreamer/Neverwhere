...
const char *trade_letters[NUM_TRADERS+1] = {
	...
	"Must be Paladin",
        ...
        "Must be Ranger",
        "Must be Barbarian",
	"Must be Aracne Archer",
	"Must be Arcane Trickster",
	"Must be Archmage",
	"Must be Assassin",
	"Must be Blackguard",
...
int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
  ...
  if ((IS_WIZARD(ch) && NOTRADE_WIZARD(shop_nr)) ||
      ...
      (IS_FIGHTER(ch) && NOTRADE_FIGHTER(shop_nr)) ||
      (IS_ARCANE_ARCHER(ch) && NOTRADE_ARCANE_ARCHER(shop_nr)) ||
      (IS_ARCANE_TRICKSTER(ch) && NOTRADE_ARCANE_TRICKSTER(shop_nr)) ||
      (IS_ARCHMAGE(ch) && NOTRADE_ARCHMAGE(shop_nr)) ||
...
char *times_message(struct obj_data *obj, char *name, int num)
{
  ...
  if (obj) {
    ...
    if ((ptr = strchr(name, '.')) == NULL)
      ...
      ptr++;
    len = snprintf(buf, sizeof(buf), "%s", ptr);
  }
