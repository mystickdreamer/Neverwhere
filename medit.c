...
void medit_setup_new(struct descriptor_data *d)
{
  ...
  SET_BIT_AR(MOB_FLAGS(mob), MOB_AUTOBALANCE);
...
void medit_disp_menu(struct descriptor_data *d)
{
  ...
  "@gC@n) Num HP Dice: [@c%4d@n],  @gD@n) Size HP Dice: [@c%5d@n],  @gE@n) HP Bonus: [@c%5d@n]\r\n"
 ...
 */
  ...
  case MEDIT_AC:
    GET_ARMOR(OLC_MOB(d)) = LIMIT(i, 10, 100);
    if (MOB_FLAGGED(OLC_MOB(d), MOB_AUTOBALANCE)) {
      TOGGLE_BIT_AR(MOB_FLAGS(OLC_MOB(d)), MOB_AUTOBALANCE);
    }
    break;
  ...
  case MEDIT_LEVEL:
    GET_HITDICE(OLC_MOB(d)) = LIMIT(i, 1, CONFIG_LEVEL_CAP);
    /* Try to add some baseline defaults based on level choice. */
    if (MOB_FLAGGED(OLC_MOB(d), MOB_AUTOBALANCE)) {
      mob_autobalance(OLC_MOB(d));
    }
    break;
