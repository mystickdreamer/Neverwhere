...
ACMD(do_lay_hands)
{
  ...
  add_innate_timer(ch, ABIL_LAY_HANDS, SKTYPE_SKILL);
}
...
ACMD(do_stunning)
{
  ...
  if (!*arg) {
    send_to_char(ch, "Whom do you wish to use the stunning fist on?\r\n");
    return;
  }
 
  if (!HAS_FEAT(ch, FEAT_STUNNING_FIST) && GET_ADMLEVEL(ch) == ADMLVL_NONE) {
    send_to_char(ch, "You do not know how to use the stunning fist!\r\n");
    return;
  }
  ...
  if (vict == ch) {
    send_to_char(ch, "You can not use stunning fist on yourself!\r\n");
    return;
  }

  if (FIGHTING(ch) != vict) {
    send_to_char(ch, "But you aren't even fighting them!\r\n");
    return;
  }

  //check uses per day

  /* Adjust DC 1/2 level and Wis Bonus */
  dc += (GET_LEVEL(ch)/2 + ability_mod_value(GET_WIS(ch)));

  /* vict make fort save vs DC */
  roll = GET_SAVE(vict, SAVING_FORTITUDE) + rand_number(1, 20);
  mudlog(BRF, ADMLVL_IMMORT, TRUE, "DEBUG - roll [%d] vs. dc [%d]", roll, dc);
  if ( roll > dc) {
    send_to_char(ch, "Your attempt to use stunning fist has failed!\r\n");
    /* decrement uses per day */
    return;
  }

  send_to_char(ch, "You focus your ki into your fist!\r\n");
  /* vict failed save, now they are stunned if strike hits */
  SET_BIT_AR(AFF_FLAGS(ch), AFF_STRIKE_STUNNING);
