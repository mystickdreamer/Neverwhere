...
void game_loop(socket_t cmmother_desc)
{
  ...
  while (!circle_shutdown) {
    ...
    if (descriptor_list == NULL) {
      ...
      if (select(cmmother_desc + 1, &input_set, (fd_set *) 0, (fd_set *) 0, NULL) < 0) {
	if (errno == EINTR)
    ...
    *prompt = '\0';
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && len < sizeof(prompt)) {
      ...
      if (PRF_FLAGGED(d->character, PRF_DISPEXP) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%d EXP ", GET_EXP(d->character));
        if (count >= 0)
          len += count;
      }
      if (PRF_FLAGGED(d->character, PRF_DISPTNL) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%d TNL ", level_exp(GET_LEVEL(d->character) + 1) - GET_EXP(d->character));
        if (count >= 0)
          len += count;
      }
...
int process_output(struct descriptor_data *t)
{
  ...
  if (t->snoop_by)
    write_to_output(t->snoop_by, "@c%%@n %*s@c%%%%@n", result, t->output);
 ...
 */
  ...
  while (nl_pos != NULL) {
    ...
    *write_point = '\0';
    ...
    if (t->snoop_by)
      write_to_output(t->snoop_by, "@c%%@n %s\r\n", tmp);
    failed_subst = 0;
...
void close_socket(struct descriptor_data *d)
{
  ...
  if (d->character) {
    ...
    if (IS_PLAYING(d) || STATE(d) == CON_DISCONNECT) {
      ...
      mudlog(CMP, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "Losing player: %s.", GET_NAME(d->character) ? GET_NAME(d->character) : "<null>");
	 ...
	 struct obj_data *obj, const void *vict_obj, int type)
{
  ...
  if (type == TO_VICT) {
    if ((to = (const struct char_data *) vict_obj) != NULL && SENDOK(to) && (!resskill || (roll_skill(to, resskill) >= dcval))) {
